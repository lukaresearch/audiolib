function Draw_FIR_LP_Sym(b)
% Draw_FIR_LP_Sym(b)
% Draw FIR Linear-Phase Structure given the Symmetric impulse response b
...
if Modd
    ...
    % Draw the last structure for b(N+1)
    x0 = (N-1)*0.75+1; x1 = x0+0.75;
    plot([x0,x1,x1,x0],[0,0,-1.5,-1.5],'ko-','linewidth',1,...
        'markersize',3,'MarkerFaceColor','k');
    plot([x1],[-0.75],'kV','MarkerFaceColor','k');
    plot([x0+0.375],[0],'k>','MarkerFaceColor','k');
    text([x0+0.375],[0.1],'{\itz}^{-1}','fontname','times',...
        'fontsize',10,'horizontalalignment','center',...
        'verticalalignment','bottom');
    ...
    text([x0+0.375],[-1.4],'{\itz}^{-1}','fontname','times',...
        'fontsize',10,'horizontalalignment','center',...
        'verticalalignment','bottom');
    plot([x1,x1],[-1.5,-2.5],'ko-','linewidth',1,...
        'markersize',3,'MarkerFaceColor','k');
    plot(x1,-2,'kV','MarkerFaceColor','k');
    text(x1-0.1,-2,sprintf('%.4g',b(N+1)),'fontname','symbol',...
                'fontsize',8,'horizontalalignment','right');
    plot([x1-0.125,x1+0.5],[-2.5,-2.5],'k-','linewidth',1);
    plot(x1+0.5,-2.5,'ko','markersize',3,'MarkerFaceColor','k');
    plot(x1+0.25,-2.5,'k>','MarkerFaceColor','k');
    text(x1+0.6,-2.5,'{\ity}({\itn})','fontname','times','fontsize',10,...
        'horizontalalignment','left','verticalalignment','middle');
end
