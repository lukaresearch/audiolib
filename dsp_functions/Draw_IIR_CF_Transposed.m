function Draw_IIR_CF_Transposed(b0,B,A)
colordef white;
b0 = round(b0*10^4)/(10^4); B = round(B*10^4)/(10^4); A = round(A*10^4)/(10^4);
k = size(A,1); xl = 0; xh = 2+k*2.5+1; yl = -2.25; yh = 0.5;
plot([1 2+k*2.5],[0 0],'ko-','linewidth',1,'markersize',3,...
    'MarkerFaceColor','k');
hold on; plot(1.5,0,'k>','MarkerFaceColor','k')
% Loop on biquads
for j = 1:k,
    Draw_IIR_Biquad_Transposed(B(j,:),A(j,:),1+2.5*(j-1),0);
end
text(0.9,0,'{\itx}({\itn})','fontname','times','fontsize',10,...
    'horizontalalignment','right','verticalalignment','middle');
text(2.1+2.5*k,0,'{\ity}({\itn})','fontname','times','fontsize',10,...
